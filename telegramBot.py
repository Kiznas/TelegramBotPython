...
async def create(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if last_event_data:
        await try_delete_message(update._bot, chat_id=last_event_data.ChatId, message_id=last_event_data.MessageId)
        LastEvent.pop(chat_id)
        ...
        if user_response == "Header":
            ...
            if(user_response == "Yes"):
                ...
                if last_event is not None:
                    await try_delete_message(context.bot, last_event.ChatId, last_event.MessageId)
                ...
                last_event : Type[LastEventData] = LastEvent[user_data.ChatId]
...
async def format_for_editing(user_data: UserData, header_text: str, description_text: str, card_data: CardData, response_text: str) -> str:
    ...
    formatted_date_gmt = gmt_time.strftime("%d %B %H:%M")
    formatted_user_response = f"{user_mention} <i>{formatted_date_gmt}</i>"

    tuple_key = (user_data.ChatId, user_data.MessageId)

    if response_text is not None:
        if tuple_key not in RespondsOnEvent:
            RespondsOnEvent[tuple_key] = RespondData([], user_data.UserId, user_data.ChatId, user_data.MessageId, formatted_date_gmt)
        ...
        if respond.Responds and any(response.startswith(user_mention) for response in respond.Responds) and response_text.lower() == "no":
            ...
            formatted_user_response = f"<s>{user_mention} <i>{formatted_date_gmt}</i></s>"
